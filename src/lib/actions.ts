...
export async function updateModule(data: z.infer<typeof updateModuleSchema>) {
  ...
  try {
    ...
    // Use the standard server-side updateDoc
...
export async function addModule(
  ...
  try {
    ...
    const newModuleData = {
        ...validation.data,
        rfloDate: validation.data.rfloDate || "",
        shipmentNo: validation.data.shipmentNo || "",
        yardReport: validation.data.yardReport || "",
        islandReport: validation.data.islandReport || "",
        signedReport: validation.data.signedReport ?? false,
    };
    const modulesCollection = collection(firestore, 'modules');
    // Use the standard server-side addDoc
